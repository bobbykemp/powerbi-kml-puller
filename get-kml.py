    ...
    for placemark in document.iter('{http://www.opengis.net/kml/2.2}Placemark'):
      ...
      for description in placemark.iter('{http://www.opengis.net/kml/2.2}description'):
        ...
        if testing_pattern.search(test_text) is not None:
          ...
          doc_dict['data_type'] = 'GPS Interference Testing'
          yield doc_dict
        # GPS outage data
...
f = open('//10.100.1.248/htix/PowerBI/Outage Reporting/kmls/' + kml_file_name, "w")
...
csv_file_name = f'outages-{kmlid}-129001450.csv'
doc_df.to_csv('//10.100.1.248/htix/PowerBI/Outage Reporting/csvs/' + csv_file_name)
